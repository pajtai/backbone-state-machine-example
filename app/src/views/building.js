define([
    ...
    var UP = Floor.prototype.masks.UP_MASK,
        ...
        Building = Backbone.View.extend({
            ...
            elevatorTransition: elevatorTransition
    });

    function initialize() {
        this.numberOfFloors = this.options.numberOfFloors;
        this.elevator = this.options.elevator;
        this.listenTo(this.elevator, "transition", this.elevatorTransition);
    ...
    function elevatorTransition(description) {
        var movement = description.current,
            floor = this.elevator.model.get(CURRENT_FLOOR),
            buttonMask = movement === MOVING_UP ? UP : DOWN,
            model;
        if (floor) {
            model = this.floors[floor].model;
            model.set(BUTTON_PRESSED, model.get(BUTTON_PRESSED) ^ buttonMask);
        }
    }
